...
• Durée d'affichage : ${typeof params.textDuration1 === 'number' ? `${params.textDuration1}s (durée EXACTE fournie par l'utilisateur — à respecter strictement)` : `À DÉDUIRE INTELLIGEMMENT par le modèle en fonction du nombre de mots, du rythme de lecture confortable (≈ 0,35s/mot, minimum 2s, maximum ${Math.max(2, videoDuration - 1)}s) et de la dramaturgie de la scène. Annoncer la durée déduite explicitement dans le storyboard.`}
• Le texte doit rester parfaitement lisible pendant TOUTE sa durée d'affichage, sans chevauchement avec d'autres overlays.` : ''}
...
• Durée d'affichage : ${typeof params.textDuration2 === 'number' ? `${params.textDuration2}s (durée EXACTE fournie par l'utilisateur — à respecter strictement)` : `À DÉDUIRE INTELLIGEMMENT en fonction du nombre de mots (≈ 0,35s/mot, minimum 2s) et du rôle CTA / punchline finale. Annoncer la durée déduite explicitement dans le storyboard.`}
• AUCUN chevauchement avec le Texte 1 : le Texte 2 commence APRÈS la disparition complète du Texte 1, ou occupe une zone visuelle clairement distincte.` : ''}
