...
${params.contentType === 'image' || params.contentType === 'carousel' ? `Direction artistique premium (OBLIGATOIRE — À INCLURE TEL QUEL, ADAPTÉ AU PRODUIT) :
[INSÉRER DANS LE PROMPT_FR ce bloc directif intégral, en l'adaptant intelligemment au produit/secteur — JAMAIS l'omettre, JAMAIS le résumer] :
"Rendu niveau agence créative top mondiale (Behance / Awwwards / Cannes Lions), digne d'un directeur artistique senior. Composition travaillée et réfléchie : grille maîtrisée, équilibre asymétrique, layering, profondeur, hiérarchie visuelle forte. Typographie display impactante en MAJUSCULES (sans-serif condensé bold OU serif éditorial OU script display selon le secteur — JAMAIS Arial/Times basique), mix intelligent de 2 polices (1 display + 1 neutre), jeu typographique (mot répété en arrière-plan transparent, lettrage qui passe devant/derrière le produit, contour ou ombre décalée colorée). Éléments graphiques OBLIGATOIRES sélectionnés selon le produit : badge prix circulaire, étoile brutaliste, splash, blob, ruban, tag oblique, bulle BD, traits manuscrits, cercles dessinés à la main, flèches stylisées, soulignés. Textures subtiles (papier, grain, halftone, noise). Ombres portées réalistes longues, lueurs, éclats. Micro-particules contextuelles (sésame, sauce qui coule, fumée, vapeur, ingrédients en lévitation, éclaboussures, gouttes de condensation). Palette saturée et assumée de 2-3 couleurs + 1 accent vif contrastant (jaune néon, orange électrique, rouge sang, vert citron) pour les badges et CTA — JAMAIS pastel fade. Produit en HERO occupant 40-60% de la surface, parfaitement détouré ou mis en scène léchée, lumière studio premium, ombres dramatiques, micro-détails désirables (brillance, texture, gouttes). Look final = AFFICHE PUBLICITAIRE PRO d'une marque internationale (références : campagnes Coca-Cola burger, Tropic Addict, XO Chinese, Jimbo, Hey grapefruit, Special Pizza, Luosifen — choisir la plus adaptée au produit et s'en inspirer pour la structure, sans copier). INTERDIT : fond uni plat sans texture, produit centré seul sans contexte graphique, typographie unique fade, absence totale de badge/forme/accent, look 'template Canva basique', composition symétrique ennuyeuse, palette pastel sans contraste, rendu simpliste ou amateur."${params.contentType === 'carousel' ? `\nAjouter pour le CARROUSEL : "Système de design UNIFIÉ sur les ${params.slidesCount || 4} slides — même palette, même typographie, même style d'éléments décoratifs, même traitement lumière. Chaque slide doit pouvoir être publiée seule comme une affiche pro."` : ''}
