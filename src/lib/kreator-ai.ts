...
export async function generatePrompt(params: {
  ...
  const videoModels = [
    ...
    'grok-imagine', 'grok-imagine-i2v', 'grok-imagine-t2v', 'grok-imagine-1.5-preview',
...
🎨 Design texte : [TOUJOURS RENSEIGNÉ — JAMAIS "aucun". L'IA DÉDUIT systématiquement un design typographique signé, niveau UI/agence de communication premium internationale (référence d'inspiration : direction artistique d'agences type McDonald's, Burger King, Tesla, Apple, Nike, Coca-Cola — codes graphiques iconiques, hiérarchie forte, identité visuelle marquée, modernité scroll-stopping). POLICE et COULEUR HEX TOUJOURS DÉDUITES par l'IA (jamais choisies par l'utilisateur), parfaitement cohérentes avec l'idée choisie, la direction artistique, la palette/branding du produit, le secteur et le ton. Position selon le réglage utilisateur si fourni, sinon déduite pour un impact maximal. CONTRASTE OBLIGATOIRE FORT entre la couleur du texte et la zone d'arrière-plan exacte sous le texte (ratio WCAG ≥ 4.5:1, viser 7:1) — INTERDIT de choisir une couleur identique ou proche du fond (même famille / même luminosité) qui empêcherait la lecture. Si fond clair → texte sombre ; si fond sombre → texte clair ; si fond complexe/texturé → ajouter ombre portée, contour ou voile subtil sous le texte pour garantir une lisibilité instantanée. Lisibilité mobile parfaite, hiérarchie forte (titre / accroche / CTA différenciés par taille, poids et couleur). Animations d'apparition stylées (fade, slide, scale, kinetic typography) cohérentes avec le rythme de la scène. Timing & durée d'apparition : valeurs utilisateur si fournies (minimum 2s), sinon déduites intelligemment selon le rythme narratif avec un MINIMUM ABSOLU DE 2s par texte (jamais d'affichage flash inférieur à 2s).${!params.showText ? ` NOTE : l'utilisateur n'a pas fourni de wording — si la scène ne comporte aucun texte écran ("aucun" déclaré ci-dessus), décrire tout de même le SYSTÈME typographique signature qui s'appliquerait à tout texte de la direction artistique (police, couleurs, traitement) afin de garder une identité visuelle homogène sur l'ensemble de la vidéo.` : ''}]
🎬 Animation texte (entrée + sortie) : [TOUJOURS RENSEIGNÉ — JAMAIS "aucun". Décrire PRÉCISÉMENT pour CHAQUE texte affiché à l'écran : (1) ANIMATION D'ENTRÉE — type d'apparition (fade-in, slide-in depuis haut/bas/gauche/droite, scale-up, blur-to-sharp, typewriter, kinetic typography lettre par lettre, mask reveal, clip-path wipe, bounce-in, rotate-in, etc.), durée d'entrée en secondes (0.3s à 0.8s), easing (ease-out, spring, cubic-bezier premium), timing exact de déclenchement (t = Xs après le début de la scène) ; (2) PHASE STATIQUE — durée d'affichage stable lisible (minimum 2s) avec éventuel micro-mouvement subtil (légère oscillation, parallax, pulse) ; (3) ANIMATION DE SORTIE — type de disparition (fade-out, slide-out, scale-down, blur-out, mask-out, glitch-out, etc.), durée de sortie (0.3s à 0.6s), easing, timing exact de fin (t = Ys avant fin de scène). Les animations d'entrée et de sortie doivent être COHÉRENTES avec la direction artistique, le rythme musical, le mood de la scène et le style de la marque (référence agence : Apple = élégance fluide, Nike = impact rapide, McDonald's = pop énergique). Chaque texte DOIT obligatoirement avoir une entrée ET une sortie animées — JAMAIS de cut brut, JAMAIS d'apparition statique, JAMAIS de disparition par cut.${!params.showText ? ` Si "Texte écran : aucun" sur cette scène, décrire tout de même le SYSTÈME d'animation typographique signature qui s'appliquerait pour garder une cohérence globale sur la vidéo.` : ''}]
...
export async function generateVideo(
  ...
  const isKieModel = [
    ...
    'grok-imagine', 'grok-imagine-i2v', 'grok-imagine-t2v',
