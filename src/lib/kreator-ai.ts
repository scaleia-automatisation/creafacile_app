...
${params.logoEnabled && params.logoUrl
  ? (params.contentType === 'video'
    ...
    : `🏷️ LOGO DE MARQUE (POST-TRAITEMENT OBLIGATOIRE — PRIORITÉ ABSOLUE) : NE PAS dessiner, recréer, styliser, écrire ni intégrer de logo dans l'image générée. Le vrai logo PNG importé par l'utilisateur sera appliqué APRÈS génération en surimpression exacte par l'application, posé directement à la position définie (${params.logoPosition === 'bottom-right' ? 'en bas à droite' : params.logoPosition === 'bottom-left' ? 'en bas à gauche' : params.logoPosition === 'top-left' ? 'en haut à gauche' : params.logoPosition === 'top-right' ? 'en haut à droite' : params.logoPosition === 'top-center' ? 'en haut au centre' : params.logoPosition === 'middle-left' ? 'au milieu à gauche' : params.logoPosition === 'middle-right' ? 'au milieu à droite' : 'en bas au centre'}).\n• URL technique du logo utilisateur (à conserver comme référence de workflow, PAS à redessiner par le modèle) : ${params.logoUrl}\n• ⛔️ NE PAS réserver de zone vide, de bande, de marge spéciale ni de cadrage particulier pour le logo. Composer librement la scène en pleine surface comme s'il n'y avait pas de logo — le logo sera simplement déposé par-dessus à sa position exacte, sans modifier la composition.\n• INTERDICTION ABSOLUE : aucun autre logo, monogramme, pictogramme de marque, lettrage de marque, enseigne ou faux logo ne doit apparaître dans l'image.`)
